...
public class CreateModuleAction extends AnAction {
  private static final Logger LOG = Logger.getInstance(CreateModuleAction.class.getName());
  ...
  public void actionPerformed(@NotNull AnActionEvent e) {
    ...
    String name = Messages.showInputDialog(project, "Add new module", "Name",
        MarionetteCoffeeIcons.MODULE_16, MarionetteCoffeeConstants.MODULE, null);
    if (name == null)
      return;
    ...
    if (!createStructure(path, upperName, lowerName, e.getProject()))
      return;
    ...
    PsiFile viewFile = componentDir.findFile(MarionetteCoffeeConstants.CONFIG_COFFEE);
    if (viewFile != null)
      view.selectElement(viewFile);
  }

  @SuppressWarnings("Duplicates")
  private boolean createStructure(@NotNull final String path, @NotNull final String upperName,
    ...
    try {
      ...
      URL url = CreateCompositeViewAction.class.getClassLoader().getResource("templates/module/");
      FileUtils.copyResourcesRecursively(url, f, false);
      final Processor<String, String> processor = new Processor<String, String>() {
        @Override
        public String process(String value) {
          value = value.replace(MarionetteCoffeeConstants.TEMPLATE_MODULENAME_LOWER, lowerName);
          return value.replace(MarionetteCoffeeConstants.TEMPLATE_MODULENAME_UPPER, upperName);
        }
      };
